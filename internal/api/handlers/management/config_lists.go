...
func (h *Handler) PutInteractionsKeys(c *gin.Context) {
	...
	if errUnmarshal := json.Unmarshal(data, &arr); errUnmarshal != nil {
		...
		if errObjUnmarshal := json.Unmarshal(data, &obj); errObjUnmarshal != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
		}
		arr = obj.Items
...
func (h *Handler) PatchInteractionsKey(c *gin.Context) {
	...
	if targetIndex == -1 && body.Match != nil {
		...
		for i := range h.cfg.InteractionsKey {
			if match != "" && h.cfg.InteractionsKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
...
func (h *Handler) PutXAIKeys(c *gin.Context) {
	...
	var entries []config.XAIKey
	if errUnmarshal := json.Unmarshal(data, &entries); errUnmarshal != nil {
		var body struct {
			Items []config.XAIKey `json:"items"`
		}
		if errObject := json.Unmarshal(data, &body); errObject != nil || len(body.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
		}
		entries = body.Items
	}
	filtered := make([]config.XAIKey, 0, len(entries))
	for i := range entries {
		entry := entries[i]
		normalizeCodexKey(&entry)
		if entry.BaseURL == "" {
...
func (h *Handler) DeleteXAIKey(c *gin.Context) {
	...
	if apiKey := strings.TrimSpace(c.Query("api-key")); apiKey != "" {
		if baseRaw, hasBase := c.GetQuery("base-url"); hasBase {
			baseURL := strings.TrimSpace(baseRaw)
		...
		matchCount := 0
		for i := range h.cfg.XAIKey {
			if strings.TrimSpace(h.cfg.XAIKey[i].APIKey) == apiKey {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
	...
	if indexRaw := c.Query("index"); indexRaw != "" {
		var index int
		if _, errScan := fmt.Sscanf(indexRaw, "%d", &index); errScan == nil && index >= 0 && index < len(h.cfg.XAIKey) {
			h.cfg.XAIKey = append(h.cfg.XAIKey[:index], h.cfg.XAIKey[index+1:]...)
			h.cfg.SanitizeXAIKeys()
			h.persistLocked(c)
			return
		}
