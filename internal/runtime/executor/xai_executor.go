...
func newXAIIncompleteStreamError() xaiIncompleteStreamError {
	return xaiIncompleteStreamError{statusErr: statusErr{code: http.StatusRequestTimeout, msg: xaiIncompleteStreamMessage}}
}

func (xaiIncompleteStreamError) IsRequestScoped() bool { return true }
...
func (e *XAIExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		errStatus := xaiStatusErr(httpResp.StatusCode, data)
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(line, xaiDataTag) {
				...
				for i, eventData := range eventDataList {
					eventData = restoreXAINamespaceToolCalls(eventData, prepared.namespaceTools)
					eventType := gjson.GetBytes(eventData, "type").String()
					terminalEvent := eventType == "response.completed" || eventType == "response.incomplete"
					if terminalErr, ok := codexTerminalFailureErr(eventData); ok {
						if errClear := clearXAIReasoningReplayOnInvalidSignature(ctx, prepared.replayScope, terminalErr.StatusCode(), eventData); errClear != nil {
							select {
					...
					if hasPendingEventLine {
						eventLine := []byte("event: " + gjson.GetBytes(eventData, "type").String())
						if i == 0 {
							eventLine = xaiNormalizeReasoningSummaryEventLine(pendingEventLine, gjson.GetBytes(eventData, "type").String())
							pendingEventLine = nil
						}
...
// OAuth defaults to the CLI chat proxy; API-key credentials default to the
// official API. The explicit using_api setting takes precedence.
...
func xaiStatusErr(code int, body []byte) statusErr {
	...
	codeText := strings.ToLower(strings.TrimSpace(gjson.GetBytes(body, "code").String()))
	errorText := strings.ToLower(strings.TrimSpace(gjson.GetBytes(body, "error").String()))
	if errorText == "" {
		errorText = strings.ToLower(strings.TrimSpace(gjson.GetBytes(body, "message").String()))
	}
	if strings.Contains(codeText, "free-usage-exhausted") ||
		strings.Contains(errorText, "free-usage-exhausted") ||
		strings.Contains(errorText, "included free usage") ||
		strings.Contains(strings.ToLower(string(body)), "free-usage-exhausted") ||
		strings.Contains(strings.ToLower(string(body)), "included free usage") {
...
func xaiSupportsReasoningEffort(model string) bool {
	...
	return info != nil && info.Thinking != nil && len(info.Thinking.Levels) > 0
}
