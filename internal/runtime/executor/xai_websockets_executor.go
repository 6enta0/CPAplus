...
func (s *xaiWebsocketIDState) mapDownstreamToUpstream(downstreamID, upstreamID string) {
	downstreamID = strings.TrimSpace(downstreamID)
	if s == nil || downstreamID == "" {
		return
	}
...
func (s *xaiWebsocketIDState) prependTranscriptInput(payload []byte) []byte {
	...
	merged := append(prefix, xaiJSONRawMessages(gjson.GetBytes(payload, "input"))...)
	updated, err := sjson.SetRawBytes(payload, "input", xaiMarshalRawMessages(merged))
	if err != nil {
		return payload
	}
	return updated
...
func (s *xaiWebsocketIDState) recordTranscriptTurn(requestPayload, completedPayload []byte) {
...
func xaiJSONRawMessages(result gjson.Result) []json.RawMessage {
	...
	out := make([]json.RawMessage, 0, len(result.Array()))
	...
	go func() {
		...
		for {
			...
			if wsErr, ok := parseXAIWebsocketError(payload); ok {
				...
				if errClear := clearXAIReasoningReplayOnInvalidSignature(ctx, prepared.replayScope, statusCode, payload); errClear != nil {
...
func buildXAIWebsocketCompactionPayload(payload, transcriptInput []byte) ([]byte, error) {
	if len(payload) == 0 {
		payload = []byte(`{}`)
	}
	if len(transcriptInput) == 0 {
	...
	out, err := sjson.SetRawBytes(bytes.Clone(payload), "input", transcriptInput)
	if err != nil {
		return nil, err
	}
...
func websocketSessionTargetChanged(sess *codexWebsocketSession, authID, wsURL string) bool {
	if sess == nil {
		return false
	}
	sess.connMu.Lock()
...
func closeXAIWebsocketSessionConnection(sess *codexWebsocketSession, reason string) {
	if sess == nil {
		return
	}
	sess.connMu.Lock()
	...
	logXAIWebsocketDisconnected(sess.sessionID, authID, wsURL, reason, nil)
	if errClose := conn.Close(); errClose != nil {
