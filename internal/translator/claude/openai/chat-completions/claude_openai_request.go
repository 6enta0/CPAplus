		...
		messages.ForEach(func(_, message gjson.Result) bool {
			...
			case "system":
				...
				if contentResult.Exists() && contentResult.Type == gjson.String && contentResult.String() != "" {
					...
					textPart = translatorcommon.AttachCacheControl(textPart, message)
					...
					contentResult.ForEach(func(_, part gjson.Result) bool {
						if part.Get("type").String() == "text" {
							...
							textPart = translatorcommon.AttachCacheControl(textPart, part)
							out, _ = sjson.SetRawBytes(out, "system.-1", textPart)
						}
						return true
					})
					if message.Get("cache_control").Exists() {
						systemParts := gjson.GetBytes(out, "system").Array()
						if len(systemParts) > systemStart && !systemParts[len(systemParts)-1].Get("cache_control").Exists() {
							path := fmt.Sprintf("system.%d", len(systemParts)-1)
							block := translatorcommon.AttachCacheControl([]byte(systemParts[len(systemParts)-1].Raw), message)
			...
			case "user", "assistant":
				...
				msg = translatorcommon.AttachMessageCacheControl(msg, message)
	...
	if tools := root.Get("tools"); tools.Exists() && tools.IsArray() && len(tools.Array()) > 0 {
		...
		tools.ForEach(func(_, tool gjson.Result) bool {
			if tool.Get("type").String() == "function" {
				...
				anthropicTool = translatorcommon.AttachCacheControl(anthropicTool, tool)
				if !gjson.GetBytes(anthropicTool, "cache_control").Exists() {
					anthropicTool = translatorcommon.AttachCacheControl(anthropicTool, function)
				}
...
func convertOpenAIContentPartToClaudePart(part gjson.Result) string {
	...
	return string(translatorcommon.AttachCacheControl(claudePart, part))
}
