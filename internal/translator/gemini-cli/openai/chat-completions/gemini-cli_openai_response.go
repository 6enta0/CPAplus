...
func ConvertCliResponseToOpenAI(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	isFinalChunk := params.UpstreamFinishReason != "" && gjson.GetBytes(rawJSON, "response.usageMetadata").Exists()
	if isFinalChunk {
		finishReason := "stop"
		if params.SawToolCall {
			finishReason = "tool_calls"
		} else if params.UpstreamFinishReason == "MAX_TOKENS" {
			finishReason = "max_tokens"
		}
		template, _ = sjson.SetBytes(template, "choices.0.finish_reason", finishReason)
		template, _ = sjson.SetBytes(template, "choices.0.native_finish_reason", strings.ToLower(params.UpstreamFinishReason))
	}
