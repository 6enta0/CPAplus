...
func ConvertOpenAIRequestToInteractions(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	out, _ = sjson.SetBytes(out, "model", interactionsRequestFirstNonEmpty(modelName, root.Get("model").String()))
...
func appendOpenAIMessageToInteractions(out []byte, message gjson.Result) []byte {
	...
	case "assistant":
		if reasoning := message.Get("reasoning_content"); reasoning.Exists() {
			for _, text := range interactionsRequestReasoningTexts(reasoning) {
				out, _ = sjson.SetRawBytes(out, "input.-1", interactionsRequestTextStep("thought", text))
			}
		}
		...
		if toolCalls := message.Get("tool_calls"); toolCalls.Exists() && toolCalls.IsArray() {
			toolCalls.ForEach(func(_, toolCall gjson.Result) bool {
				if step, ok := interactionsRequestToolCallStep(toolCall); ok {
					out, _ = sjson.SetRawBytes(out, "input.-1", step)
				}
				return true
			})
...
func openAIChatContentPartToInteractions(part gjson.Result) ([]byte, bool) {
	...
	case "input_audio", "audio":
		...
		data := interactionsRequestFirstNonEmpty(audio.Get("data").String(), part.Get("data").String())
		if data == "" {
			return nil, false
		}
		out, _ = sjson.SetBytes(out, "data", data)
		if format := interactionsRequestFirstNonEmpty(audio.Get("format").String(), part.Get("format").String()); format != "" {
	...
	case "file", "input_file", "document":
		...
		if filename := interactionsRequestFirstNonEmpty(file.Get("filename").String(), part.Get("filename").String()); filename != "" {
			out, _ = sjson.SetBytes(out, "filename", filename)
		}
		if data := interactionsRequestFirstNonEmpty(file.Get("file_data").String(), part.Get("file_data").String(), part.Get("data").String()); data != "" {
			out, _ = sjson.SetBytes(out, "data", data)
		}
		if url := interactionsRequestFirstNonEmpty(file.Get("file_url").String(), part.Get("file_url").String(), part.Get("url").String()); url != "" {
...
func openAIChatImagePartToInteractions(part gjson.Result) []byte {
	out := []byte(`{"type":"image"}`)
	imageURL := interactionsRequestFirstNonEmpty(part.Get("image_url.url").String(), part.Get("image_url").String(), part.Get("url").String())
...
func openAIToolResultToInteractions(message gjson.Result) []byte {
	...
	if callID := interactionsRequestFirstNonEmpty(message.Get("tool_call_id").String(), message.Get("id").String()); callID != "" {
...
func openAIChatToolToInteractions(tool gjson.Result) ([]byte, bool) {
	...
	name := interactionsRequestFirstNonEmpty(tool.Get("function.name").String(), tool.Get("name").String())
	if name == "" {
		return nil, false
	}
