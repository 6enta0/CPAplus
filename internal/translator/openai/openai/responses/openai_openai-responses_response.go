...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	emitPendingFunctionArgs := func(key string) {
		...
		argsBuffer := st.FuncArgsBuf[key]
		if argsBuffer == nil || argsBuffer.Len() <= st.FuncArgsSent[key] {
			return
		}
		arguments := argsBuffer.String()
		delta := arguments[st.FuncArgsSent[key]:]
		callID := st.FuncCallIDs[key]
		...
		event, _ = sjson.SetBytes(event, "item_id", fmt.Sprintf("fc_%s", callID))
		event, _ = sjson.SetBytes(event, "output_index", st.FuncOutputIx[key])
		event, _ = sjson.SetBytes(event, "delta", delta)
		out = append(out, emitRespEvent("response.function_call_arguments.delta", event))
		st.FuncArgsSent[key] = len(arguments)
	}
			...
			if delta.Exists() {
				...
				if tcs := delta.Get("tool_calls"); tcs.Exists() && tcs.IsArray() {
					...
					tcs.ForEach(func(_, tc gjson.Result) bool {
						...
						if callID := tc.Get("id").String(); callID != "" && st.FuncCallIDs[key] == "" {
							st.FuncCallIDs[key] = callID
						}
						if name := tc.Get("function.name").String(); name != "" && !st.FuncItemAdded[key] {
							st.FuncNames[key] = name
						}
						if arguments := tc.Get("function.arguments"); arguments.Exists() && arguments.String() != "" {
							st.FuncArgsBuf[key].WriteString(arguments.String())
						}
		...
		choices.ForEach(func(_, choice gjson.Result) bool {
			...
			if msg.Exists() {
				...
				if tcs := msg.Get("tool_calls"); tcs.Exists() && tcs.IsArray() {
					...
					tcs.ForEach(func(toolIndex, tc gjson.Result) bool {
						callID := tc.Get("id").String()
						if callID == "" {
							callID = fmt.Sprintf("call_%s_%d_%d", id, choice.Get("index").Int(), toolIndex.Int())
						}
