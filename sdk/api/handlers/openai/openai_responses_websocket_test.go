...
func TestForwardResponsesWebsocketTreatsResponseDoneAsTerminalWithoutRewriting(t *testing.T) {
	...
	if got := gjson.GetBytes(payload, "type").String(); got != wsEventTypeDone {
		t.Fatalf("payload type = %s, want %s; payload=%s", got, wsEventTypeDone, payload)
	}
...
func TestResponsesWebsocketUsesCodexWebsocketPassthroughFalseWhenMixedBackends(t *testing.T) {
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(&websocketDirectCaptureExecutor{})
	auths := []*coreauth.Auth{
		{ID: "auth-codex", Provider: "codex", Status: coreauth.StatusActive, Attributes: map[string]string{"websockets": "true"}},
		{ID: "auth-openai", Provider: "openai-compatibility", Status: coreauth.StatusActive, Attributes: map[string]string{"websockets": "true"}},
	}
	for _, auth := range auths {
		if _, err := manager.Register(context.Background(), auth); err != nil {
			t.Fatalf("Register auth %s: %v", auth.ID, err)
		}
		registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "mixed-model"}})
	}
	t.Cleanup(func() {
		for _, auth := range auths {
			registry.GetGlobalRegistry().UnregisterClient(auth.ID)
	...
	base := handlers.NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, manager)
	h := NewOpenAIResponsesAPIHandler(base)
	if h.responsesWebsocketUsesCodexWebsocketPassthrough("mixed-model") {
		t.Fatal("mixed provider pool unexpectedly enabled Codex websocket passthrough")
	}
}

func TestResponsesWebsocketUsesCodexWebsocketPassthroughFalseWhenCodexAuthLacksWebsocket(t *testing.T) {
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(&websocketDirectCaptureExecutor{})
	auth := &coreauth.Auth{ID: "auth-codex-sse", Provider: "codex", Status: coreauth.StatusActive}
	if _, err := manager.Register(context.Background(), auth); err != nil {
		t.Fatalf("Register auth: %v", err)
	}
	registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "codex-sse-model"}})
	t.Cleanup(func() {
		registry.GetGlobalRegistry().UnregisterClient(auth.ID)
	})
...
func TestResponsesWebsocketStripsGenerateWhenWebsocketAttemptFallsBackToHTTP(t *testing.T) {
	...
	if got := executor.AuthIDs(); len(got) != 3 || got[0] != "auth-ws" || got[1] != "auth-http" || got[2] != "auth-http" {
		t.Fatalf("selected auth IDs = %v, want [auth-ws auth-http auth-http]", got)
	}
	...
	if len(httpPayloads) != 2 {
		t.Fatalf("auth-http payload count = %d, want 2", len(httpPayloads))
	}
	if gjson.GetBytes(httpPayloads[0], "generate").Exists() {
		t.Fatalf("generate leaked after HTTP fallback: %s", httpPayloads[0])
	}
	if gjson.GetBytes(httpPayloads[1], "previous_response_id").Exists() {
		t.Fatalf("previous_response_id leaked after HTTP fallback repin: %s", httpPayloads[1])
	}
	secondInput := gjson.GetBytes(httpPayloads[1], "input").Array()
	if len(secondInput) != 3 || secondInput[0].Get("id").String() != "msg-1" || secondInput[1].Get("id").String() != "out-http" || secondInput[2].Get("id").String() != "msg-2" {
		t.Fatalf("HTTP fallback repin did not receive merged transcript: %s", httpPayloads[1])
	}
}
...
func TestShouldReleaseResponsesWebsocketPinnedAuth(t *testing.T) {
	cases := []struct {
		...
		{name: "bad gateway", err: &interfaces.ErrorMessage{StatusCode: http.StatusBadGateway, Error: fmt.Errorf("upstream failed")}, want: true},
		...
		{name: "gateway timeout", err: &interfaces.ErrorMessage{StatusCode: http.StatusGatewayTimeout, Error: fmt.Errorf("upstream timed out")}, want: true},
...
func TestResponsesWebsocketReleasesPinnedAuthAfterPrematureStreamClose(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	storedAuthA, ok := manager.GetByID("auth-a")
	if !ok || storedAuthA == nil || storedAuthA.Status != coreauth.StatusActive || storedAuthA.Unavailable {
		t.Fatalf("premature stream close mutated auth-a status: %+v", storedAuthA)
	}

	replayPayloads := executor.Payloads("auth-b")
	if len(replayPayloads) != 1 {
		t.Fatalf("auth-b replay payload count = %d, want 1", len(replayPayloads))
	}
	replayPayload := replayPayloads[0]
